function frac_dim = boxcount_no_moisy(coordmat, maxstep, plotflag)
...
if (plotflag) 
  % figure;
  %plot(x, y, 'mo', ...
   %    [x(1), x(end)], [param(1)*x(1) + param(2), param(1)*x(end) + param(2)], 'm-');
   plot(x, y, 'ko');hold on
   plot([x(1), x(end)], [param(1)*x(1) + param(2), param(1)*x(end) + param(2)], 'ro-');
