...
%
% Pair dispersion processing for position and velocity
%
% A loop line 98 can run in parallel to improve time execution.
...
y=vertcat(tracks.y);
z=vertcat(tracks.z);
 vx=vertcat(tracks.u); % compute using velocities
 vy=vertcat(tracks.v);
 vz=vertcat(tracks.w);
t=vertcat(tracks.t);
