function bkg = getBkg(subfolders,Istart,Iend,increment,demosaicpattern)


fname = [subfolders filesep 'frame_' num2str(Istart,'%06d') '.tiff' ];
...
for k = Istart : increment : Iend
    Nim=Nim+1;
    fname = [subfolders filesep 'frame_' num2str(k,'%06d') '.tiff' ];
    if ~isempty(demosaicpattern)
